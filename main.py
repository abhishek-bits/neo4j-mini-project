...
class Neo4JDriver:
    ...
    def get_all_author_ids(self):
        ...
        return self._read_cypher_query(self, cypher)
    ...
    # # Given two author IDs, delete CO_AUTHOR relationship
    def delete_co_author_relationship(self, from_author_id, to_author_id):
        cypher = "MATCH (a:Author)-[r:CO_AUTHOR]->(b:Author) " \
                 "WHERE ID(a) = " + str(from_author_id) + " " \
                    "AND ID(b) = " + str(to_author_id) + " " \
                 "DELETE r"
        self._write_cypher_query(self, cypher)

    # Deletes all CO_AUTHOR relationships from the Database
    # WARNING! This is dangerous :D
    def drop_co_author_relationship(self):
        cypher = "MATCH ()-[r:CO_AUTHOR]->() " \
                 "DELETE r"
        self._write_cypher_query(cypher)

    # Deletes all AUTHOR relationships from the Database
    # WARNING! This is dangerous :D
    def drop_author_relationship(self):
        cypher = "MATCH ()-[r:AUTHOR]->()" \
                 "DELETE r"
        self._write_cypher_query(cypher)

    # Deletes all Authors from the Database
    # Can't delete an author unless all its relationships (i.e. AUTHOR, CO_AUTHOR) are deleted
    # WARNING! This is dangerous :D
    def drop_authors(self):
        cypher = "MATCH (a:Author) " \
                 "DELETE a"
        self._write_cypher_query(cypher)

    @staticmethod
    def _read_cypher_query(self, cypher):
...
# Implements Depth First Traversal on a Vertex Disconnected Graph
